...
export const Food2D = ({ type, size = 100 }: Food2DProps) => {
  ...
  if (type === 'dot') {
    return (
      <div style={containerStyle} className="flex items-center justify-center">
        <div 
            className="w-[25%] h-[25%] rounded-sm shadow-sm transition-colors duration-300 ease-in-out" 
            style={{ backgroundColor: pelletThemeColor }}
        />
      </div>
    );
  }
  ...
  if (type === 'power') {
    return (
      <div style={containerStyle} className="flex items-center justify-center">
        <div 
          className={`
            w-[65%] h-[65%] rounded-full transition-all duration-200 
            ${isPowerPelletVisible ? 'opacity-100 scale-100' : 'opacity-70 scale-90'}
          `}
          style={{ 
              backgroundColor: pelletThemeColor, 
              boxShadow: isPowerPelletVisible ? `0 0 15px ${pelletThemeColor}` : 'none' 
          }} 
        />
      </div>
    );
  ...
  if (type === 'cherry') {
    return (
      <svg style={containerStyle} viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" aria-label="Cherry">
        {/* Stems */}
  ...
  if (type === 'strawberry') {
    return (
      <svg style={containerStyle} viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" aria-label="Strawberry">
        {/* Strawberry Body Shape */}
        <path 
          d="M50 95 C 20 70, 5 50, 15 30 C 20 20, 35 20, 50 35 C 65 20, 80 20, 85 30 C 95 50, 80 70, 50 95 Z" 
          fill="#ef4444" 
          stroke="#b91c1c" 
          strokeWidth="2" 
