...
import { Group, Shape, ExtrudeGeometry, MathUtils } from 'three'; 
...
export const Food3D = ({ consumableVariant }: Food3DProps) => {
    ...
    return (
      <group ref={meshGroupRef}>
        <mesh rotation={[0.5, 0.5, 0]}>
          ...
          <meshStandardMaterial color={themeDerivedPelletColor} emissive={themeDerivedPelletColor} emissiveIntensity={0.5} roughness={0.3} />
        </mesh>
      </group>
    );
  }
    ...
    return (
      <group ref={meshGroupRef}>
        <mesh>
          ...
          <meshStandardMaterial color={themeDerivedPelletColor} emissive={themeDerivedPelletColor} emissiveIntensity={2} toneMapped={false} />
        </mesh>
  ...
  if (consumableVariant === 'cherry') {
    ...
    return (
      <group ref={meshGroupRef} position={[0, 0.1, 0]}>
        <group position={[-0.16, -0.2, 0]} rotation={[0, 0, 0.2]}><mesh><sphereGeometry args={[0.15, 32, 32]} /><meshStandardMaterial {...cherryFleshMaterial} /></mesh></group>
        <group position={[0.16, -0.2, 0]} rotation={[0, 0, -0.2]}><mesh><sphereGeometry args={[0.15, 32, 32]} /><meshStandardMaterial {...cherryFleshMaterial} /></mesh></group>
        <mesh position={[-0.08, 0.08, 0]} rotation={[0, 0, -0.3]}><cylinderGeometry args={[0.008, 0.008, 0.6]} /><meshStandardMaterial {...stemMaterial} /></mesh>
        <mesh position={[0.08, 0.08, 0]} rotation={[0, 0, 0.3]}><cylinderGeometry args={[0.008, 0.008, 0.6]} /><meshStandardMaterial {...stemMaterial} /></mesh>
        <mesh position={[0, 0.36, 0]}><sphereGeometry args={[0.018]} /><meshStandardMaterial {...stemMaterial} /></mesh>
      </group>
    );
  }
    ...
    return (
      <group ref={meshGroupRef} scale={[0.9, 0.9, 0.9]}>
        <mesh position={[0, 0, 0]} scale={[0.95, 1.15, 0.95]}>
            ...
            <MeshDistortMaterial color={strawberryRed} roughness={0.4} metalness={0.1} distort={0.25} speed={0} />
        </mesh>
        <group position={[0, 0.22, 0]}>
            <mesh position={[0, 0.05, 0]}><cylinderGeometry args={[0.03, 0.04, 0.1]} /><meshStandardMaterial color={leafGreen} roughness={0.8} /></mesh>
            {[0, 72, 144, 216, 288].map((degreeAngle, index) => (
                <group key={index} rotation={[0.4, MathUtils.degToRad(degreeAngle), 0]}>
                     <mesh position={[0, 0.02, 0.08]} scale={[1, 0.1, 1.5]} rotation={[-0.2, 0, 0]}><sphereGeometry args={[0.08, 16, 16]} /><meshStandardMaterial color={leafGreen} roughness={0.7} side={2} /></mesh>
                </group>
            ))}
        </group>
        {PRECALCULATED_STRAWBERRY_SEEDS.map((position, index) => (
            <mesh key={`seed-${index}`} position={position}><sphereGeometry args={[0.012, 8, 8]} /><meshStandardMaterial color={seedYellow} roughness={0.5} /></mesh>
        ))}
      </group>
    );
  }
  ...
  if (consumableVariant === 'life') {
    return (
      <group ref={meshGroupRef} scale={[0.2, 0.4, 0.4]} rotation={[Math.PI, 0, 0]} position={[0, 0.2, 0]}>
