...
import { Eyes3D } from './Game/3D/Ghosts/Eyes3D'; // <--- ახალი იმპორტი
...
export const GhostShowcase = () => {
  ...
  // პოზიციის გასწორება ვარიანტების მიხედვით
  let yPos = 0;
  if (variant === 2) yPos = 0.6; // Reaper უფრო მაღლა დგას
  if (variant === 3) yPos = 0.3; // თვალები შუაში
      ...
      >
        ...
        <div>
            ...
            <div className="grid grid-cols-1 gap-2">
                ...
                {/* Variant 3: Eyes (NEW) */}
                <button
                    onClick={() => updateSetting('ghostVariant', 3)}
                    className={`p-4 rounded-lg text-left transition-all border flex items-center ${
                        variant === 3
                ...
                >
                    <span className="text-xl mr-3">👀</span> Eaten Eyes
                </button>
            </div>
        </div>

        {/* მხოლოდ მაშინ ვაჩვენებთ ფერის არჩევას, თუ თვალები არაა არჩეული (რადგან თვალებს ფერი არ ეცვლება) */}
        {variant !== 3 && (
            <div>
                <h3 className={`text-sm uppercase tracking-wider mb-3 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
                აირჩიე ფერი 
